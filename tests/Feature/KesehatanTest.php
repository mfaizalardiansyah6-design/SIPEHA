...
class KesehatanTest extends TestCase
{
    ...
    public function test_pemeriksaan_page_renders_with_blok_filter_and_form(): void
    {
        ...
        $this->actingAs($this->petugas)
            ...
            ->assertOk()
            ->assertSee('Catat Pemeriksaan Baru')
            ->assertSee('Cari blok...')
            ->assertSee('Cari WBP / No. Reg...');
    }

    public function test_pemeriksaan_page_preserves_blok_filter(): void
    {
        Wbp::factory()->create(['nama' => 'ADI SAPUTRA', 'blok_kamar' => 'A1', 'no_register' => 'REG-001']);

        $this->actingAs($this->petugas)
            ->get(route('monitoring.pemeriksaan-kesehatan', ['blok' => 'A1', 'q' => 'ADI']))
            ->assertOk()
            ->assertSee('A1');
    }

    public function test_legacy_log_status_maps_to_sudah_label(): void
    {
        ...
        $this->actingAs($this->petugas)
            ->get(route('monitoring.pemeriksaan-kesehatan'))
            ->assertOk()
            ->assertSee('status_label\u0022:\u0022Sudah', false)
            ->assertSee('status\u0022:\u0022Hadir', false);
    ...
    public function test_pemeriksaan_set_status_still_saves_for_wbp(): void
    {
        $wbp = Wbp::factory()->create();
    ...
    public function test_pemeriksaan_set_status_requires_wbp(): void
    {
        $this->actingAs($this->petugas)
            ->post(route('monitoring.set-status'), [
                'category_id' => $this->category->id,
