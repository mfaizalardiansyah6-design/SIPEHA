<x-app-layout title="Layanan Laundry">
    ...
    <div class="grid grid-cols-1 gap-6 xl:grid-cols-3" x-data="laundryTable(@js($tablePayload))">
                            ...
                            <tr class="hover:bg-canvas-parchment/50 transition-colors">
                                ...
                                <td class="table-td">
                                    <template x-if="w.status">
                                        <span class="badge" :class="w.fulfilled ? 'badge-green' : 'badge-blue'" x-text="w.status"></span>
                                    </template>
                                    <template x-if="!w.status">
        ...
        <div class="space-y-6">
            <div class="card card-padding" x-data="laundryForm(@js(route('monitoring.wbp-search')), @js($statusOptions), @js($blokList))">
                <h2 class="mb-1">Catat Laundry Baru</h2>
                <p class="text-sm text-muted mb-4">Cari blok, pilih WBP, lalu simpan status laundry.</p>
                ...
                <form method="POST" :action="@js(route('monitoring.set-status'))" x-ref="form" class="space-y-4">
                    ...
                    <div>
                        <x-input-label for="laundry-blok" value="Blok/Kamar" />
                        <div class="relative" @click.outside="blokOpen = false">
                            <template x-if="!selectedBlok">
                                <input type="text"
                                       id="laundry-blok"
                    ...
                    <div>
                        <x-input-label for="laundry-wbp" value="Pilih WBP" />
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <x-input-label for="laundry-tanggal" value="Tanggal" />
                            <input type="date" id="laundry-tanggal" name="tanggal" value="{{ $tanggal }}" class="form-input" required>
                        </div>
                        <div>
                            <x-input-label for="laundry-status" value="Status" />
                            <select id="laundry-status" name="status" x-model="status" class="form-input" required>
                                @foreach ($statusOptions as $option)
                                    <option value="{{ $option }}">{{ $option }}</option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        <x-input-label for="laundry-keterangan" value="Keterangan" />
                        <textarea id="laundry-keterangan" name="keterangan" rows="2" class="form-input" placeholder="opsional"></textarea>
                    </div>
            ...
            <div class="card card-padding">
                ...
                <div class="flex flex-wrap items-center gap-2 mt-4" x-show="distribusi.length > 0">
                    <template x-for="[status, count] in distribusi" :key="status">
                        <span class="badge" :class="status === 'Selesai' ? 'badge-green' : 'badge-blue'" x-text="status + ': ' + count"></span>
                    </template>
                </div>
            </div>
        </div>
