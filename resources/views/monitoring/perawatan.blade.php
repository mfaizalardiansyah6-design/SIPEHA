<x-app-layout :title="$category->nama_layanan">
            ...
            <div class="flex flex-wrap items-center justify-end gap-2">
                ...
                <div class="flex items-center gap-1 rounded-lg bg-ink/5 p-1 overflow-x-auto max-w-full">
                    @foreach ($tabs as $slug => $label)
                        <a href="{{ route('monitoring.perawatan', ['tab' => $slug, 'tanggal' => $tanggal, 'blok' => $blok, 'q' => $q]) }}"
                           data-tab-link
    ...
    <div class="grid grid-cols-1 gap-6 xl:grid-cols-3" x-data="perawatanTable(@js($tablePayload))">
        <div class="card card-padding xl:col-span-2">
            <div class="flex flex-wrap items-center justify-between gap-4 mb-4">
                <div>
                    <h2>{{ $category->nama_layanan }} — {{ \Carbon\Carbon::parse($tanggal)->translatedFormat('d M Y') }}</h2>
                            ...
                            <tr class="hover:bg-canvas-parchment/50 transition-colors">
                                ...
                                <td class="table-td">
                                    <template x-if="w.status">
                                        <div>
                                            <span class="badge" :class="w.fulfilled ? 'badge-green' : 'badge-orange'" x-text="w.status"></span>
                                            <p class="text-xs text-muted mt-1" x-text="w.log_date"></p>
                                        </div>
                                    </template>
                                    <template x-if="!w.status">
                                ...
                                <td class="table-td">
                                    <form method="POST" :action="setStatusUrl">
                                        ...
                                        <select name="status" x-model="w.status" class="form-input !w-32 !py-1.5 text-xs" @change="$event.target.value ? $event.target.form.submit() : null">
                        ...
                        <template x-if="filtered.length === 0">
                            <tr>
                                <td colspan="4" class="px-6 py-12 text-center text-muted">
                                    <x-icon name="shirt" class="mx-auto w-10 h-10 mb-2 opacity-50" />
        ...
        <div class="space-y-6">
            <div class="card card-padding" x-data="perawatanForm(@js(route('monitoring.wbp-search')), @js($statusOptions), @js($blokList))">
                <h2 class="mb-1">Catat Perawatan Baru</h2>
                <p class="text-sm text-muted mb-4">Cari blok, pilih WBP, lalu simpan status perawatan diri.</p>
                ...
                <form method="POST" :action="@js(route('monitoring.set-status'))" x-ref="form" class="space-y-4">
                    ...
                    <div>
                        <x-input-label for="perawatan-blok" value="Blok/Kamar" />
                        <div class="relative" @click.outside="blokOpen = false">
                            <template x-if="!selectedBlok">
                                <input type="text"
                                       id="perawatan-blok"
                    ...
                    <div>
                        <x-input-label for="perawatan-wbp" value="Pilih WBP" />
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <x-input-label for="perawatan-tanggal" value="Tanggal" />
                            <input type="date" id="perawatan-tanggal" name="tanggal" value="{{ $tanggal }}" class="form-input" required>
                        </div>
                        <div>
                            <x-input-label for="perawatan-status" value="Status" />
                            <select id="perawatan-status" name="status" x-model="status" class="form-input" required>
                                @foreach ($statusOptions as $option)
                                    <option value="{{ $option }}">{{ $option }}</option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        <x-input-label for="perawatan-keterangan" value="Keterangan" />
                        <textarea id="perawatan-keterangan" name="keterangan" rows="2" class="form-input" placeholder="opsional"></textarea>
                    </div>
            ...
            <div class="card card-padding">
                <h2 class="mb-4">Ringkasan Perawatan</h2>
                ...
                <div class="flex flex-wrap items-center gap-2 mt-4" x-show="distribusi.length > 0">
                    <template x-for="[status, count] in distribusi" :key="status">
                        <span class="badge" :class="status === 'Selesai' ? 'badge-green' : 'badge-orange'" x-text="status + ': ' + count"></span>
                    </template>
                </div>
            </div>
        </div>
